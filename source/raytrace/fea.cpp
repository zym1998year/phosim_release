...
void Fea::knnQueryFit(std::vector<double> & x, std::vector<double> & y, double *z, double *z_r, double *z_phi, size_t nNear, int degree) {
    ...
    void Fea::knnQueryFitDegree1(std::vector<double> & x, std::vector<double> & y, double *z, double *z_a, double *z_b, int *z_flag, size_t nNear, int coordinate) {
        int coordType = coordinate; //1: Cartesian  2: polar
    int outputCoordType = coordinate; //1: Cartesian 2: polar
    //coordType=2;
    int sameNeighbors = 0;
    ...
    for (size_t t(0); t < x.size(); t++) {
        ...
        if (sameNeighbors == 0) {
            ...
            for  (size_t n(0); n < nNear; n++) {
                ...
                b[n] = m_z[idx];
                if (m_z[idx] < 10e-3 && m_z[idx] > -10e-3) {

                } else {
                    // printf("Error 1: %lf %ld\n",m_z[idx],idx);
                }
                s0 += 1;
                sx += a1;
                sy += a2;
        ...
        for (int d(1); d <= degree; d++) {
            for (int dp(0); dp <= d; dp++) {
                if (coordType == 1) {
                    ...
                    z[t] += c[m]*zernike(d, dp, r, phi);
                    if (outputCoordType == 2) {
                        z_a[t] += c[m]*zernike_r(d, dp, r, phi)*rmax;
                        z_b[t] += c[m]*zernike_phi(d, dp, r, phi);
                    } else {
                        z_a[t] += c[m]*zernike_r(d, dp, r, phi)*rmax * x[t]/r - c[m]*zernike_phi(d, dp, r, phi) * y[t]/r/r;
                        z_b[t] += c[m]*zernike_r(d, dp, r, phi)*rmax * y[t]/r + c[m]*zernike_phi(d, dp, r, phi) * x[t]/r/r;
                    }
                }
                m++;
            }
        ...
        if (coordType == 1) {
            if (r>0 && r<rmax) {
                if (outputCoordType==2) {
                    z_a[t] += (z_x*x[t]/r + z_y*y[t]/r)*rmax;     // dz/dr = dz/dx cos(theta) + dz/dy sin(theta)
                    z_b[t] += r*(-z_x*y[t]/r + z_y*x[t]/r);     // dz/dtheta = r [ -dz/dx sin(theta) + dz/dy cos(theta) ]
                } else {
                    z_a[t] += z_x*rmax;
                    z_b[t] += z_y*rmax;
                }
        ...
        if (z[t] < 10e-3 && z[t] > -10e-3) {

        } else {
            // printf("Error 2: %lf %ld\n",z[t],t);
        }
        if (badflag==1) {
            z_a[t]=0.0;
            z_b[t]=0.0;
