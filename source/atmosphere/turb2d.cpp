...
void* Atmosphere::threadFunction(void *voidArgs) {
    thread_args *args = (thread_args*)voidArgs;

    args->instance->turb2d(args->seed, args->see5, args->outerx, args->outers, args->zenith, args->wavelength, args->name, args->N_size, args->resample);
    return NULL;
}

void Atmosphere::turb2d(long seed, double see5, double outerx, double outers,
                        double zenith, double wavelength,
                        char *sname, long N_size, double resample) {
    ...
    r0 = (0.98 * (wavelength*1e4*1.0e-8)/ ( (3.1425926/(180.0*60.0*60.0))
                                            * (newsee)*pow(1/cos(zenith*PI/180.),0.6)*wavelengthfactor) );
    ...
    double range = 1024.0;
    double scaleResample = pow(resample, 1.0 + 5.0/3.0);
    ...
    for (int k = 0; k < 4; k++) {
        ...
        for (int ix = 0; ix < nx; ix++) {
            ...
            for (int iy=0; iy < ny; iy++) {
                ...
                if (dkk > (range/scale)/sqrt(2.0)/sqrt(powerbuffer) ||
                    dkk <= (range/scale_next)/sqrt(2.0)/sqrt(powerbuffer)) {
                    dkkp = 0.0;
                }
        ...
        for (int i=0; i < N; i++) {
            in_full[i] = inc_full[i][0]/static_cast<double>(N)/scaleResample;
            in_high[i] = inc_high[i][0]/static_cast<double>(N)/scaleResample;
            in_low[i] = inc_low[i][0]/static_cast<double>(N)/scaleResample;
        }
